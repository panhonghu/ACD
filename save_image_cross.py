...
save_dir = os.path.join('./save_images', "ConditionalDiffusion_no_adv")
...
for idx, (img, img_HF, label, cam, path, item) in enumerate(train_loader):
    ...
    for i in range(bs):
        ...
        if path_i_item[3] in ['cam1', 'cam2', 'cam4', 'cam5']:  # rgb->ir
            ...
            rgb2ir_dir = os.path.join(save_dir, 'fake', path_i_item[-3], path_i_item[-2])
            if os.path.exists(rgb2ir_dir):
                pass
            else:
                os.makedirs(rgb2ir_dir)
            rgb2ir_path = os.path.join(rgb2ir_dir, path_i_item[-1])
            # network propagation
            rgb_i = img_i
            rgb_HF_i = img_HF_i
            target_i = label[i]
            ...
            rgb2ir_i = rgb2ir.detach().cpu().squeeze(0)
            ...
            rgb2ir_i = torch.cat((rgb2ir_i[2,:,:].unsqueeze(0), \
                                  rgb2ir_i[1,:,:].unsqueeze(0), \
                                  rgb2ir_i[0,:,:].unsqueeze(0)), dim=0)
        ...
        elif path_i_item[3] in ['cam3', 'cam6']:  # ir->rgb
            # for the true visible images
            ...
            # for the generated infrared images
            ir2rgb_dir = os.path.join(save_dir, 'fake', path_i_item[-3], path_i_item[-2])
            if os.path.exists(ir2rgb_dir):
                pass
            else:
                os.makedirs(ir2rgb_dir)
            ir2rgb_path = os.path.join(ir2rgb_dir, path_i_item[-1])
            # network propagation
            ir_i = img_i
            ir_HF_i = img_HF_i
            target_i = label[i]
            DiffusionModel.set_inputs(ir_i, ir_HF_i, ir_i, ir_HF_i, target_i, target_i)
            ir2rgb = DiffusionModel.generator.sample(x0=DiffusionModel.ir, con_x=DiffusionModel.ir_HF, modality="rgb")
            ir2rgb_i = ir2rgb.detach().cpu()
            ir_i = ir_i.detach().cpu().squeeze(0)
            ir2rgb_i = ir2rgb_i.detach().cpu().squeeze(0)
            ...
            ir2rgb_i = torch.cat((ir2rgb_i[2,:,:].unsqueeze(0), \
                                  ir2rgb_i[1,:,:].unsqueeze(0), \
                                  ir2rgb_i[0,:,:].unsqueeze(0)), dim=0)
...
for idx, (img, img_HF, label, cam, path, item) in enumerate(gallery_loader):
    ...
    for i in range(num_images_one_iter):
        ...
        # for the generated infrared images
        rgb2ir_dir = os.path.join(save_dir, 'fake', rgb_name[-3], rgb_name[-2])
        if os.path.exists(rgb2ir_dir):
            pass
        else:
            os.makedirs(rgb2ir_dir)
...
for idx, (img, img_HF, label, cam, path, item) in enumerate(query_loader):
    ...
    for i in range(num_images_one_iter):
        ...
        # for the generated infrared images
        ir2rgb_dir = os.path.join(save_dir, 'fake', ir_name[-3], ir_name[-2])
        if os.path.exists(ir2rgb_dir):
            pass
        else:
            os.makedirs(ir2rgb_dir)
